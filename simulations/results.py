...
class SimulationResults:
    ...
    def collect_calculate_data(self, base_station, users):
        ...
        self.total_time_throughput = throughputCalculator.calculate_TotalTime_throughput()
        self.theoretical_throughput = throughputCalculator.calculate_theoretical_throughput()
        for user in users:
            if user.user_id == 0:
                user.plot()
            ...
            self.efficiency.append(user.mcs['efficiency'])  # 取调制阶数，例如 2, 4, 16, 64
            self.bler_values.append(user.bler)  # 误块率
            self.snr_values.append(user.snr)  # 信噪比
            self.modulation.append(user.mcs['modulation'])  # 取调制方式，例如 QPSK, 16QAM, 64QAM
            # 平均单个TTI的传输次数
            self.transmission_time.append(user.transmission_time / CONFIG.simulation.tti_length)
        ...
        self.avg_bler = BLERCalculator(self.modulation, self.bler_values).calculate_avg_bler()
        self.avg_delay = DelayCalculator(self.transmission_time).calculate_average_delay()

    def plot_throughput_distribution(self):
        """绘制吞吐量分布图"""
        plt.figure(figsize = (8, 5))
        plt.bar(self.user_ids, np.array(self.throughputs) / (10 ** 6), color = 'blue', alpha = 0.7)
        plt.xlabel("用户 ID")
        plt.ylabel("吞吐量 (Mbps)")
        plt.title("用户吞吐量分布")
        plt.grid()
        if CONFIG.simulation.save_info:
            ...
            plt.savefig(os.path.join(results_dir, 'throughput distribution.png'))
        plt.show()

    def plot_cqi_distribution(self):
        """绘制 CQI 分布情况"""
        plt.figure(figsize = (8, 5))
        plt.hist(self.cqi_values, bins = np.arange(1, 16) - 0.5, edgecolor = 'black', alpha = 0.7)
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'CQI distribution.png'))
        plt.show()
    ...
    def plot_mcs_distribution(self):
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'MCS distribution.png'))
        plt.show()
    ...
    def show_hist_snr(self):
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'Final SNR distribution.png'))
        plt.show()
    ...
    def show_bler_analysis(self):
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'BLER data.png'))
        plt.show()
    ...
    def print_summary_stats(self):
        ...
        print(f"时域上平均-仿真总吞吐量: {time_mean_throughput / 1e9:.4f} "
              f"Gbps,达理论值的{rate:.2%}")
        ...
        if CONFIG.simulation.save_info:
            ...
            with open(file_path, 'w') as f:
                ...
                f.write(f"BLER>10%的用户比例: {np.mean(np.array(self.bler_values) > 0.1):.2%}\n")
                ...
                f.write(f"时域上平均-仿真总吞吐量: {time_mean_throughput / 1e9:.4f} "
                        f"Gbps,达理论值的{rate:.2%}\n")
    ...
    def show_delay_distribution(self):
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'delay distribution.png'))
        plt.show()
    ...
    def show_snr_mcsindex(self):
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'SNR-MCS.png'))
        plt.show()
    ...
    def show_snr_efficiency(self):
        ...
        if CONFIG.simulation.save_info:
            ...
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            plt.savefig(os.path.join(results_dir, 'SNR-Efficiency.png'))
        plt.show()
